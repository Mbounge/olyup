...
const TimeCell = ({
  ...
  const timeCallBack = (inputCell) => {
    ...
    if (typeof trainingSession.trainingSession !== undefined) {
      ...
      if (cellIndex >= 0) {
        ...
        if (trainingSession.trainingSession[cellIndex].hasOwnProperty('time')) {
          console.log('KOOL');
          ...
          if (tallyIndex >= 0) {
            trainingSession.trainingSession[cellIndex].time.data[tallyIndex] = {
              value: inputCell.input.data.value,
              tally: inputCell.input.data.tally,
            };
            // aggregate here
            var data = 0;
            trainingSession.trainingSession[cellIndex].time.data.forEach(
              (item) => {
            ...
            setCalculation(data);
            trainingSession.trainingSession[cellIndex].time['calculation'] =
              data;
          } else if (tallyIndex == -1) {
            trainingSession.trainingSession[cellIndex].time.data.push({
              value: inputCell.input.data.value,
              tally: inputCell.input.data.tally,
            });
            // aggregate here
          ...
          !trainingSession.trainingSession[cellIndex].hasOwnProperty('time')
        ) {
          console.log('IT AINT HERE!!!');
          trainingSession.trainingSession[cellIndex]['time'] = {
            data: [
              {
                value: inputCell.input.data.value,
                tally: inputCell.input.data.tally,
          ...
          console.log(`I am about to kill somebody ${data}`);
          setCalculation(data);
          trainingSession.trainingSession[cellIndex].time['calculation'] = data;
        }
      } else if (cellIndex == -1) {
  ...
  useEffect(() => {
    handleTimeChange();
    coreCallback({ type: 'time', value: calculation });
  }, [sets, calculation]);
